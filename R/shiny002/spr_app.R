...
# draw.data<-function(input){
#   dtaout<-dta_setx(set,input)
# 
#   }
...
#  tb1<-getmean(dta,c(0,0,0,0,sm,em),0,0,0,1,ti) # with RAW dataset
  ...
  plot_desc<-function(setd){
   ...
   # p <- ggplot(data=bar_df,mapping=aes(x=group,y=LZ,fill=RT)) 
    ...
    #testausgabe der funktion:
   # print(head(d5$timeinterval))
  #}
  ###### here output of set of checkboxed input
#########
#    proof_desc(dta_setx(dta,c(0,1,-1,1,sm,vso),1,1,1)) #nur mit glt=1 (x,1,x)
#  dataout<-dta_setx(dta,c(input[1],input[2],input[3],1,sm,vso),input[4],input[5],input[6]) #nur mit glt=1 (x,1,x)
#}
  ################
...
ui <- pageWithSidebar(
  ...
  # Sidebar ----
  sidebarPanel(
    # type = übergabevariablenname an draw.data
    # radioButtons("type", "Select a configuration:", 
    #              list("target -1 on" = -1,
    #                   "target -1 off" = -9,
    #                   "target +1" = 1,
...
server <- function(input, output) {
  ...
  # mydata <- reactive({
  #   draw.data(input$type) #type = einzelne buttons der abfrage
  # })
  mydata <- reactive({
