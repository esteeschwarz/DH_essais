...
```{=html}
<audio controls>
  <source src="https://potmaschin.rotefadenbuecher.de/audio/magen_musik/a-cdtrack8-d.mp3" type="audio/mpeg">
  You would hear audio but your browser does not support the audio element.
</audio>
  ```

# 1. einleitung
precondition: Im Seminar "Digitale Dramenanalyse" des SS2022 (Frank Fischer, FUB) war den Teilnehmern freigestellt, sich während des Semester einem Stück aus der DraCor Einakterdatenbank [@çakir2022] & [@einakter2022] zu widmen. Das sollte den Prozess umfassen der Transkription und Auszeichnung nach TEI Standard. Es wurden 9 Stücke selbst gewählt und, teilweise im Team, daran gearbeitet. Über den Zwischenstand wurden Referate zu Stück und Autor sowie evtl. schon während des Seminars kennengelernter Analysen vorgestellt. Ich habe Christian Gottlieb Klemms "Der Besuch" ausgesucht, bin bei der Auswahl aber nur nach Sprache **Deutsch** und einem allein zu bewältigenden
Umfang gegangen.
...
## 2.2 stück
"Der Besuch" ist ein Bühnenstück mit 5 handelnden Hauptpersonen (Celimene, Chlorinde, Cydalise, Erast, Damis) und einer Nebenfigur (Finette) sowie einer weiteren Randfigur (Ein Bedienter). Das Stück ist 1765 in der von Klemm herausgegebenen Wochenzeitschrift "Der Österreichische Patriot" zuerst, 1767 in den "Beyträgen zum deutschen Theater" ein weiteres Mal erschienen. Es ist in 9 Szenen unterteilt, zählt um 3.400 Wörter.
...
Als Textgrundlage des im folgenden *Rohtext* genannten Materials dienten die von google books öffentlich zur Verfügung gestellten scans der Erstausgabe von 1765 (A1765_Q) [@google1765], die grösztenteils auf den in der Österreichischen Nationalbibliothek schon vorhandenen Digitalisierungen [@önb1765] basieren. Diese wurden auf wikisource und hiermit auf einer platform zur kollaborativen Erstellung von u.a. Transkriptionen (vom Seminarleiter) weiterhin zugänglich gemacht und durchliefen nun den Prozess von Transkription und zweifacher unabhängiger Korrektur [@wikisource1765]. Zur Transkription diente mir im ersten Stadium eine zu 90% verläszliche OCR (optical character recognition) mittels TRANSKRIBUS [@read-coop2022]. Benutzt wurde das frei verfügbare OCR-model "ONB_Newseye_GT_M1+", das zufriedenstellende Ergebnisse lieferte -- allerdings nur mit der Ausgabe von 1767 (A1767_Q), die ich als scan in der Digitalen Sammlung der Münchner Staatsbibliothek [@mdz1767] gefunden hatte. Da sich diese, wie ich bald feststellte, doch in einigen Punkten von der Erstausgabe unterschied, wurde der Rohtext auf Wikisource also :nur unter Zuhilfenahme: dieser OCR-Fassung erstellt, d.h. im Einzelnen:
...
Der grosze Teil der weiteren Arbeit bestand in der ersten vereinfachten Auszeichnung nach TEI-Standard. Ich habe hierzu ein umfassendes script in der Programmiersprache R [@rstudio2022] entworfen, das in A1765_R mittels der Suche mit regulären Ausdrücken (z.B.[@regexr2022]) Szenenwechsel, Sprecherbezeichnungen und Regieanweisungen nach einigem Aufwand zu finden vermochte[^1] um diese schlieszlich in HTML-tags zu :verpacken: d.h. die einzelnen verschiedenen Elemente des Textes nach einem Standard zu markieren, der uns vorlag und bereits als Schema zur Validierung im Editor verwendet werden konnte [@drama2022]. Es handelte sich dabei noch nicht um die finale, schemagerechte Version. Diese wurde weiterhin vor der Integration des Stückes in das dracor-Korpus [@dracor2019] vom Seminarleiter auf Grundlage der vereinfachten Version erstellt.
