...
print(lms)
lmmax1<-which.max(lms$vcov[2:4,5])
lmmax<-round(lms$vcov[lmmax1,5],6)
cns2<-colnames(lms$vcov)[lmmax1]
cns3<-colnames(lms$vcov)[3]
cns4<-colnames(lms$vcov)[4]
lmdif1<-round(lms$vcov[5,3]-lms$vcov[5,2],6)*10^4
lmdif2<-round(lms$vcov[5,4]-lms$vcov[5,2],6)*10^4
lmdif3<-round(lmdif1-lmdif2,6)

```

Die Abhängigkeit der sentiment values vom Vorhandensein multilingualer Elemente läszt sich kurz umreiszen: Wir stellen den gröszten Zusammenhang mit [coefficient] ``r lmmax`` bei ``r cns2`` fest, die Differenz zu ``r cns3`` beträgt ``r lmdif1``, zu ``r cns4`` ``r lmdif2`` Punkte, der Abstand der Abhängigkeit hier also ``r lmdif3`` Punkte.   
