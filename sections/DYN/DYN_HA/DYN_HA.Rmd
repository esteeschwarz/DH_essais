...
#dta_t$types
#set$types
min<-min(dta_t$tokens[2:length(dta_t$tokens)])
```
...
Zum Beispiel lassen sich die Wortgleichungen visualisieren, die an bestimmten Positionen des Textes aufscheinen. Die Höhen in der folgenden Graphik markieren relative (Fourier-transformierte) Wortpositionen, an denen von Wolf die **wenigsten** analogen Wörter verwendet wurden. Es läszt sich erkennen, dasz ein Text meist mit denselben Wörtern anfängt, die immer verschiedener werden, um sich bei der Hälfte des Textes über eine lange Strecke zu gleichen und ab 60% sprunghaft zu divergieren bis sie bei 80% einen peak erreichen an Verschiedenheit.
...
```
Eine weiterhin schöne Graphik entsteht, wenn man die Matrix der Zeichenanzahl über die Gesamtheit der Wörter visualisiert. Hier zeigt sich, dasz der Text zwischen 20-40% die längsten Wörter enthält, die zur Mitte hin kürzer werden um bei 80% einen erneuten peak in der Zeichenanzahl zu erreichen.
...
#most-probable-text p {color: violet;}
```
...
### most probable text
13506.ST: Uljana Wolf P-text `r printptext()`
fin.
