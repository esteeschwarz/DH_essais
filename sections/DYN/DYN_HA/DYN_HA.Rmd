...
# zur autorin
Uljana Wolf, der Öffentlichkeit seit 2005 durch ihre Gedichte bekannt, wurde 2006 für ihr Debüt *kochanie ich habe brot gekauft* (kookbooks 2005) mit dem Peter-Huchel-Preis geehrt und veröffentlichte seitdem zwei weitere Gedichtbände, ebenfalls bei *kookbooks*. Dort ist sie in ein enges Netzwerk junger deutschsprachiger Autor* eingebunden, die sich (hier zum Aspekt *postdeutsch*) mehrheitlich durch ihre Affinität zu mehr- oder polylingualer Dichtung auszeichnen. Es gibt bei kook kaum ein* Dichter* 
# work
## corpus aufbereitung
...
### multiLX
Die multilingualen Elemente des Korpus (insgesamt `r lmlx` tokens) haben einen Anteil von `r round(length(mlx_u)/sum(dta_t$types)*100)`% an den types.
...
plot(get_lxmatches(),type = "h",xlab="text corpus",ylab="occurences")
#par(new=T)
...
```
Eine weiterhin schöne Graphik entsteht, wenn man die Matrix der Zeichenanzahl über die Gesamtheit der Wörter visualisiert. Hier zeigt sich, dasz der Text zwischen 20-40% die längsten Wörter enthält, diese zur Mitte hin kürzer werden um bei 80% einen erneuten peak in der Zeichenanzahl zu erreichen.
...
## sentiment analysis
Für jeden einzelnen Text liesz sich ein Wert bestimmen (zur Berechnung cf. [[@jockers_revealing_2015]](https://www.matthewjockers.net/2015/02/02/syuzhet/)), der Aussagen darüber zuläszt, in welchem Spektrum (positive/negative sentiment) sich dieser verorten läszt.
...
### dependencies
Mit der Regressionsanalyse des *R lme4 package* [@bates_fitting_2015] lassen sich hier Abhängigkeiten (Korrelationen) der *sentiment values* von verschiedenen Faktoren (chapter, book) aufzeigen.
...
```{r lx-sent-projection, echo=F,fig.cap="multilingual elements over sentiment",warning=FALSE}
sent_global_f<-get_transformed_values(sent_global)
