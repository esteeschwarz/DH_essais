...
hhh
### dure:
- since one friendly corrector had already styled paragraphs&speaker, would be more sense instead of using the plain text version from wikisource to start from the "html" (no real html, embedded in some strange ebook, >convert to docx >extract styling >fine.
hhh
##### fin